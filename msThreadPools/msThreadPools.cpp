...
)
{
	...
	_tprintf("MyTimerCallback: timer has fired.\n");
}
...
VOID DemoCleanupPersistentWorkTimer()
{
	...
	rollback = 1; // pool creation succeeded
				  ...
				  //
				  // The thread pool is made persisten simply by setting
	...
	rollback = 3; // Creation of work succeeded
				  ...
				  // work item (using CreateThreadpoolWork), it is guaranteed to
				  // execute
				  //
	...
	// that are members of the cleanup group, so it is not
	// neccessary to call close functions on individual objects
...
main_cleanup:
	//
	// Clean any individual pieces manually
...
VOID DemoNewRegisterWait()
{
	...
	rollback = 1; // CreatEvent succeeded

	Wait = CreateThreadpoolWait(waitcallback, NULL, NULL);

	if (Wait == NULL) {
		_tprintf(_T("CreateThreadWait failed. LastError: %u\n"), GetLastError());
	...
	// each time before signaling the event to trigger
	// the wait callback
	//
	for (i = 0; i < 5; i++) {
		...
		// Delay for the waiter thread to act if neccessary
		Sleep(500);
